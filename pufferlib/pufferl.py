...
class PuffeRL:
    ...
    def evaluate(self):
        ...
        # Set style z for RL rollouts — use expert z from VAE (trajectory-consistent)
        ...
        if style_z_dim > 0:
            ...
            agent_expert_z = getattr(self.vecenv.driver_env, '_agent_expert_z', None)
            if agent_expert_z is not None:
                # Use VAE-encoded z of each agent's GT trajectory
                z_array = agent_expert_z.copy()
            else:
                # Fallback to random z if VAE not available
    ...
    def train(self):
        ...
        for mb in range(self.total_minibatches):
            ...
            # Compute log likelihood loss of human actions under current policy (if enabled)
...
def train(env_name, args=None, vecenv=None, policy=None, logger=None):
    ...
    train_config = dict(**args["train"], env=env_name, eval=args.get("eval", {}), env_args=args.get("env", {}))
